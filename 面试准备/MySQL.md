...
+ 优化二：索引上的等值查询，向右遍历时且最后一个值不满足等值条件的时候，next-key lock退化为间隙锁（左开右开）（最后一个前面都是next-key lock，比如1, 4, 6, 10，10不符合等值条件，因此next-key lock为(1, 4], (4, 6], (6, 10) ）。（注意，范围查询不会退化！）
...
1. 由于表中没有7，单位为next-key lock因此加锁范围为(5, 10]。（在排好序的索引上寻找7,只要发现当前索引是10时才知道7没有，因此范围是(5, 10]，通过锁next指针来实现的）
...
4. session A的操作涉及 id 和 c，覆盖索引，因此锁是加在二级索引c上的（二级索引的叶子节点存储主键，等价于(c, id)），对主键索引不影响，因此session B不会受到阻塞

   > 注意session A使用的是lock in share mode，只锁覆盖索引，但如果使用for update，那么将会给主键索引上满足条件的行加上行锁。如果想在lock in share mode下也加上行锁，那么可以通过增加一个非二级索引的查询字段来绕开覆盖索引的优化。
...
1. A在主键索引上寻找，寻找到了第一个值10，本来是next-key lock (5, 10]，但是因为优化一，因此只锁了10，而没有加next-key lock，因此不会锁(5, 10)，session B的第一个插入成功。
2. A需要往后找，找到下一个值15,终止了查询条件，又因为15不符合条件，因此next-key lock (10, 15]（注意，范围查询不会退化成间隙锁！）因此session C失败。
...
这个与案例三的区别在于是非唯一索引，当唯一索引搜索到记录时，nextk-key lock会退化成行锁，而在此案例中并不能退化，因此会锁住(5, 10]。因此session B插入失败。
