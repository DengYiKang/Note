...
可以查看conf/nginx.conf，端口为80，用浏览器访问，有以下结果：
...
<img src="../pic/98.png" style="zoom: 200%;" />
...
<img src="../pic/101.png" style="zoom: 150%;" />
...
> 图中判断ETag表示是否有ETag。也就是说，服务器可能使用ETag方式，也可能使用Last-Modified方式。

<img src="../pic/442.png" style="zoom:80%;" />
...
> 异步的好处：
>
> redis缓存+异步更新数据库。因为更新数据库的耗时远远要比走redis缓存的耗时大，因此将最慢的部分异步化，这样在redis中操作成功后可以直接返回给用户，而异步使得redis与数据库能保证最终一致性。从用户的角度，只等待了redis查询的时间。
...
> 还有一个问题，因为使用的落单减库存方案，肯定是要先走redis才能判断是否有剩余库存，但是如果包裹着它的spring事务挂掉了，redis是不会回滚的。解决方案之一是将redis流程放在最后一步。
...
+ 售罄后不去操作后续流程
+ 售罄后通知各系统售罄
+ 回补上新，增加addStock的接口，将关于itemId的所有缓存清空，包括库存售罄标识，增加库存的操作直接走数据库

注意，在秒杀活动进行时，页面上的库存数是不会刷新的，因为页面走的是第一层缓存即guava cache，我们在decreaseStock时并没有同步更新guava cache，只同步更新redis和异步更新数据库。当发现库存售罄，在redis中添加库存售罄标识时，我们将各个缓存的关于库存的字段设为0，及时通知给用户。这种做法是符合情理的，因为秒杀商品的数量一般比较少，并且在短时间内由大批的用户抢购，因此在这么短的时间内我们不需要及时同步距离用户最近的缓存，只需要在售罄时更新即可，因为库存瞬间到0这种情况用户也能接受，毕竟是秒杀活动。
...
用户点击下单时每次都会发送生成令牌的请求，因此秒杀令牌只要活动一开始就无限制生成，影响系统性能。（但是需要检验信息，这种情况应该是牺牲性能换取安全性？）
...
+ 根据秒杀商品初始库存颁发对应数量令牌，控制大闸流量（发放限量令牌）
+ 用户风控策略前置到秒杀令牌发放中
+ 库存售罄判断前置到秒杀令牌发放中

#### 实现

在商品发布的接口里根据库存数定义一个最多发放令牌的次数，缓存到redis中，当次数减到0那就不再发放。

#### 缺陷

+ 浪涌流量涌入后系统无法应对。当秒杀商品的库存数有上万甚至更大，那么在短时间内大闸也无法应对这么大的流量。
+ 多库存，多商品等令牌限制能力弱。

### 队列泄洪原理

排队有些时候比并发更高效（例如redis单线程模型，innodb mutex key等）（避免并发下的调度切换消耗）。

依靠排队去限制并发流量。

依靠排队和下游拥塞窗口程度调整队列释放流量大小。（拥塞窗口即伪单线程，将拥塞窗口中的所有任务并发处理）。

#### 实现

在orderContollre里，添加线程池：
...
开放生成验证码的接口，将userId与验证码正确结果存入redis中，并将图像输出到HttpServeletResponse中。

在OrderController生成令牌之前检查验证码是否正确。
...
+ 限并发
+ 令牌桶算法
  + 一个恒定的速度往桶里放入令牌，而如果请求需要被处理，则需要先从桶里获取一个令牌，当桶里没有令牌可取时，则拒绝服务。
  + 适合突发状况。
+ 漏桶算法
  + 水（请求）先进入到漏桶里，漏桶以一定的速度出水，当水流入速度过大会直接溢出，可以看出漏桶算法能强行限制数据的传输速率。
  + 因为漏桶速率恒定，因此一般用于平滑流量，但漏桶速率恒定也意味着不适合突发。

#### 限流力度

+ 接口维度：例如OrderController的创建订单的接口
+ 总维度：每个接口都对并发能力进行了评估，但是如果在某一时刻各个接口的流量都达到了最大值，那么整个系统能否承受得住流量的总和？

#### 限流范围

+ 集群限流：依赖redis或其他的中间件技术做**统一计时器**，往往会产生性能瓶颈
+ 单机限流：负载均衡的前提下单机平均限流效果更好

#### 限流实现

使用guava的`RateLimiter`来实现令牌桶限流：
...
在创建订单接口的开头对令牌进行获取，获取失败则不进行后续操作：
...
+ 限制一个会话（session_id, token）同一秒钟/分钟接口调用多少次，但是多会话接入绕开无效。
+ 限制一个IP同一秒钟/分钟接口调用多少次，但是对于使用nat代理，多个用户对外表现的是同一个IP，容易误伤。
...
+ **innodb_file_per_table=1**：如果上百张表的数据文件都是同一个，因为是随机寻址，那么写数据将会非常耗时。如果每张表都有各自的数据文件，那么随机寻址将会非常高效。
+ **innodb_buffer_pool_size=1G**：如果buffer足够大，那么buffer缓存的命中率将会提高。
+ **innodb_log_file_size=256M**：过小将会增大新建log文件的频率，这期间所有的flush操作将会block掉，当log_buffer满了，那么所有的sql语句将会block掉；过大时将会导致数据恢复寻址困难。
...
+ **innodb_data_file_path=ibdata1:1G;ibdata2:1​G;ibdata3:1G:auto extend**：文件的分区。

### 搜索功能

+ 模糊查询：例如where name like '%name%'，是绝对不允许的，因为进行全表扫描
+ 搜索引擎：推荐
