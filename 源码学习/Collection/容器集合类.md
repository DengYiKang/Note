...
+ `final V putVal(int hash, K key, V value, boolean onlyIfAbsent, boolean evict)`：
	...
	如果table还未初始化（null或length为0），那么先resize。如果当前node为TreeNode类型，那么调用红黑树的接口，否则插入到当前node的链表上，在循环遍历next时，如果当前链表的长度（包含table上的node，即该链表的头）大于`TREEIFY_THRESHOLD`，那么将该链表进行树型化。
...
+ 与HashMap的实现不同的是LinkedHashMap使用双向链表连接所有的entry
